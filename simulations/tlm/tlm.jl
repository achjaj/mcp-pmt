...
in the original Guidicottis source code the logarithm was calculated using an
approximation which should not produce NaN values I believe =#
function ntlog(arg)
    if arg < 0
...
# this is the main function which iteratively calculates the gain
...
# writes tensor to a file
# only values from certain specified steps are dumped
...
#= the parameters are also written out because of the `tlm-explore.jl` program
the outputed parameters can be also used to check if the configuration file was
read correctly =#
